...
try:
    ...
    print("Warning: NumPy not available. Some visual filters will be disabled.")
...
def resource_path(relative_path):
    ...
    if not os.path.exists(path):
        ...
        # Create placeholder file for missing resources
...
def load_extended_data_from_firebase():
    ...
    try:
        ...
        if response.status_code == 200 and response.json():
            ...
            if 'coins' in data:
                player_coins = max(0, int(data['coins']))
...
def save_extended_data_to_firebase():
    ...
    extended_data = {
        'stats': player_stats,
        'achievements': {k: v for k, v in achievements.items() if v.get('unlocked', False)},
    ...
    try:
        ...
        if local_user_id:
            ...
            if response.status_code == 200 and response.json():
                ...
                HighScore = max(0, settings.get('high_score', 0))
...
def sync_high_score_to_firebase():
    """Sync high score immediately when achieved"""
    if not user_id:
        return
...
def update_high_score(new_score):
    """Update high score and sync to Firebase"""
    global HighScore
...
def upload_highscore(name):
    """Upload highscore with verification"""
...
# Filter functions with numpy fallback
filter_cache = {}
...
def get_cached_filtered_image(base_image, filter_func, cache_key):
    ...
    if cache_key not in filter_cache:
        if filter_func and NUMPY_AVAILABLE:
            try:
...
def apply_simple_filter(surface, tint_color, brightness_offset=0):
    """Simple filter without numpy"""
    try:
        filtered_surface = surface.copy()

        # Simple brightness adjustment
        if brightness_offset != 0:
            for x in range(filtered_surface.get_width()):
                for y in range(filtered_surface.get_height()):
                    color = filtered_surface.get_at((x, y))
                    new_color = (
                        max(0, min(255, color[0] + brightness_offset)),
                        max(0, min(255, color[1] + brightness_offset)),
                        max(0, min(255, color[2] + brightness_offset)),
                        color[3] if len(color) > 3 else 255
                    )
                    filtered_surface.set_at((x, y), new_color)

        # Apply tint
        if tint_color != (255, 255, 255):
            tint_surface = pygame.Surface(surface.get_size(), pygame.SRCALPHA)
            tint_surface.fill((*tint_color, 60))
            filtered_surface.blit(tint_surface, (0, 0), special_flags=pygame.BLEND_ALPHA_SDL2)
...
# Advanced filter functions (numpy required)
def apply_hsl_filter(surface, hue_shift=0, saturation_mult=1.0, lightness_mult=1.0):
    """HSL filter function"""
    if not NUMPY_AVAILABLE:
        return apply_simple_filter(surface, (255, 255, 255))
...
def apply_advanced_filter(surface, contrast=1.0, brightness=0, saturation=1.0, hue_shift=0, gamma=1.0):
    ...
    if not NUMPY_AVAILABLE:
        tint = (255, 255, 255)
        if hue_shift > 0:
            tint = (100, 150, 255)  # Night-like tint
        return apply_simple_filter(surface, tint, brightness)
...
def apply_night_filter(surface):
    """Night filter: darker, bluish"""
    return apply_advanced_filter(surface, contrast=1.3, brightness=-30, saturation=0.7, hue_shift=180, gamma=0.8)


def apply_desert_filter(surface):
    """Desert filter: warmer, orange/yellow"""
    return apply_advanced_filter(surface, contrast=1.1, brightness=20, saturation=1.3, hue_shift=30, gamma=1.2)


def apply_retro_filter(surface):
    """Retro filter: greenish"""
    return apply_advanced_filter(surface, contrast=0.8, brightness=-10, saturation=0.6, hue_shift=120, gamma=0.9)


def apply_neon_filter(surface):
    """Neon filter: high saturation"""
    return apply_advanced_filter(surface, contrast=1.5, brightness=10, saturation=2.0, hue_shift=280, gamma=1.1)


def apply_vintage_filter(surface):
    """Vintage filter: sepia-like"""
    return apply_advanced_filter(surface, contrast=0.9, brightness=-5, saturation=0.4, hue_shift=40, gamma=1.1)


def apply_monochrome_filter(surface):
    """Monochrome filter"""
    return apply_advanced_filter(surface, contrast=1.2, brightness=0, saturation=0.0, hue_shift=0, gamma=1.0)
...
def draw_card(surface, rect, title, content, theme_colors):
    ...
    try:
        ...
        if title:
            ...
            title_surface = font_small.render(str(title), True, title_color)
...
# Extended Themes with shop info
themes = {
    "Classic": {
...
try:
    font_path = resource_path("fonts/BaiJamjuree-Bold.ttf")
    if font_path:
...
slap_sfx = sounds["slap"]
woosh_sfx = sounds["woosh"]
score_sfx = sounds["score"]
select_sfx = sounds["select"]
coin_sfx = score_sfx  # Use score sound for coins
achievement_sfx = select_sfx  # Use select sound for achievements

# Load base images with error handling
base_loaded_images = {}
for key, path in base_images.items():
    try:
        img_path = resource_path(path)
        if img_path:
            base_loaded_images[key] = pygame.image.load(img_path).convert_alpha()
        else:
            # Create placeholder image
            placeholder = pygame.Surface((64, 64), pygame.SRCALPHA)
            placeholder.fill((255, 0, 255))  # Magenta placeholder
            base_loaded_images[key] = placeholder
            print(f"Using placeholder for: {key}")
    except Exception as e:
...
# Create coin image
try:
    coin_img_path = resource_path("images/coin.png")
    if coin_img_path:
    ...
    def draw(self, surface):
        ...
        for particle in self.particles:
            try:
                ...
                color = (*particle['color'][:3], min(alpha, 255))

                if particle['type'] == 'feather':
                    pygame.draw.ellipse(surface, color[:3],
                                        (game_area.x + particle['x'] - particle['size'],
                                         game_area.y + particle['y'] - particle['size'] // 2,
                                         particle['size'] * 2, particle['size']))
                else:
                    pygame.draw.circle(surface, color[:3],
                                       (int(game_area.x + particle['x']),
                                        int(game_area.y + particle['y'])),
                                       particle['size'])
            except:
                pass


# Ghost Recorder Class
class GhostRecorder:
    def __init__(self):
    ...
    def record_position(self, x, y):
        ...
        if self.recording:
            self.current_run.append((int(x), int(y)))

    def finish_recording(self, final_score):
        """Finish recording and save if best run"""
        self.recording = False
    ...
    def draw_ghost(self, surface, frame):
        ...
        try:
            ...
            if ghost_pos and self.best_score > 0:
                ghost_surface = base_loaded_images["player"].copy()
                ...
                ghost_surface.blit(tint_surface, (0, 0))
        ...
        if not self.collected:
            try:
                ...
                pygame.draw.circle(surface, (255, 215, 0),
                                   (int(game_area.x + self.x + 10), int(game_area.y + self.y + self.bobbing + 10)), 8)
...
def check_achievements():
    ...
    # Award coins and show notifications
...
def draw_achievement_notification():
    ...
    global achievement_display_timer, current_achievement_message
    ...
    if current_achievement_message and achievement_display_timer > 0:
        try:
            ...
            # Simple background
            pygame.draw.rect(screen, themes[current_theme]["highlight_color"], card_rect, border_radius=15)
            pygame.draw.rect(screen, themes[current_theme]["accent_color"], card_rect, 3, border_radius=15)

            # Draw text
            title_surface = font_small.render(str(current_achievement_message["title"]), True,
                                              themes[current_theme]["text_color"])
            desc_surface = font_tiny.render(str(current_achievement_message["description"]), True,
                                            themes[current_theme]["text_color"])
            reward_surface = font_tiny.render(f"+{current_achievement_message['reward']} coins!", True,
                                              (255, 215, 0))

            screen.blit(title_surface, (card_rect.x + 20, card_rect.y + 15))
            screen.blit(desc_surface, (card_rect.x + 20, card_rect.y + 45))
            screen.blit(reward_surface, (card_rect.x + 20, card_rect.y + 70))
...
# Theme loading with error handling
def load_theme_images(theme_name):
    """Load theme images with filters"""
    theme = themes[theme_name]
...
class Player:
    ...
    def update(self):
        try:
            self.velocity += 0.75  # Original gravity value
            self.y += self.velocity  # Original velocity application
...
def draw_main_menu():
    ...
    try:
        ...
        if upload_status:
            ...
            status_text = font_tiny.render(str(upload_status), True, themes[current_theme]["text_color"])
...
def draw_shop_screen():
    ...
    try:
        ...
        coins_text = font_medium.render(f"Coins: {player_coins}", True, (255, 215, 0))
        ...
        for i, theme_name in enumerate(theme_names):
            ...
            if not is_unlocked and not can_unlock:
                req_text = font_tiny.render(str(reason), True, (200, 200, 200))
...
def draw_settings_screen():
    ...
    try:
        ...
        music_label = font_small.render("Music:", True, themes[current_theme]["text_color"])
        screen.blit(music_label, (250, display_y + 20))
...
def show_admin_editor():
    ...
    try:
        ...
        print(f"Error in admin editor: {e}")


# Pipe class with coins
class Pipe:
...
def check_collision(player_obj, pipes):
    ...
    try:
        ...
        if player_obj.rect.bottom >= ACTUAL_PLAY_HEIGHT:
            return True
        ...
        for pipe in pipes:
            if pipe.x + pipe_up_img.get_width() > 0 and pipe.x < GAME_WIDTH:
                ...
                if pipe.coin and not pipe.coin.collected:
                    ...
                    if player_obj.rect.colliderect(coin_rect):
                ...
                # Pipe collision
                offset_top_x = pipe.top_rect.left - player_obj.rect.left
                offset_top_y = pipe.top_rect.top - player_obj.rect.top
                offset_bottom_x = pipe.bottom_rect.left - player_obj.rect.left
                offset_bottom_y = pipe.bottom_rect.top - player_obj.rect.top

                if player_obj.mask.overlap(pipe_down_mask, (offset_top_x, offset_top_y)):
                    return True
                if player_obj.mask.overlap(pipe_up_mask, (offset_bottom_x, offset_bottom_y)):
                    return True
...
def update_difficulty():
    ...
    try:
        ...
        return base_pipe_velocity, base_gap, 200
...
def reset_game():
    ...
    try:
        ...
        player_obj = Player(GAME_WIDTH // 4, GAME_HEIGHT // 2)
        ...
        return player_obj, pipes
    except Exception as e:
        print(f"Error in reset_game: {e}")
        return Player(GAME_WIDTH // 4, GAME_HEIGHT // 2), []
...
def handle_game_over(player_obj):
    ...
    try:
        ...
        particle_system.add_death_particles(player_obj.x + player_img.get_width() // 2,
                                            player_obj.y + player_img.get_height() // 2)
...
def main_game():
    """Enhanced main game loop with comprehensive error handling"""
    ...
    global music_enabled, current_music, player
    ...
    print("Starting enhanced Flappy Bird...")
        ...
        try:
                ...
                try:
                    ...
                    if event.type == pygame.MOUSEBUTTONDOWN:
                        ...
                        # Settings screen
                            ...
                            if fs_button and fs_button.collidepoint(mouse_pos):
                                safe_play_sound(select_sfx)
                                toggle_fullscreen()

                            if music_button and music_button.collidepoint(mouse_pos):
                                safe_play_sound(select_sfx)
                                ...
                                for theme_name, button_data in theme_buttons.items():
                                    if button_data['button'].collidepoint(mouse_pos):
                                        ...
                                        if button_data['is_current']:
                                            ...
                                            unlock_theme(theme_name)

                except Exception as e:
                    print(f"Error handling event: {e}")

            # Game Logic
            if game_state == 2 and has_moved and player:
                try:
                    player.update()
            ...
            try:
                ...
                if game_state == 1:
                    ...
                    status_text = font_small.render(str(upload_status), True, themes[current_theme]["text_color"])
                    status_rect = status_text.get_rect(center=(window_w // 2, window_h // 2 - 10))
                    screen.blit(status_text, status_rect)

                    continue_text = font_tiny.render("Press SPACE to continue", True,
                                                     themes[current_theme]["text_color"])
